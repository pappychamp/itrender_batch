...
@pytest.mark.asyncio
async def test_fetch_article_image_success(thinkit_api):
    """
    fetch_article_imageメソッドの正常テスト
    """
    # サンプルURLを用意
    sample_url = "https://thinkit.com/test/items/test"
    # サンプルHTMLを用意
    sample_html = """
        <html>
            <head>
    ...
    with aioresponses() as m:
        ...
        # テスト
        response = await thinkit_api.fetch_article_image(sample_url)
    assert response == "test_content"


@pytest.mark.asyncio
async def test_fetch_article_image_no_a_tag(thinkit_api):
    """
...
@pytest.mark.asyncio
async def test_fetch_article_image_exception(thinkit_api):
    """
    ...
    with aioresponses() as m:
        ...
        # テスト
        with pytest.raises(aiohttp.ClientError):
            await thinkit_api.fetch_article_image(sample_url)
